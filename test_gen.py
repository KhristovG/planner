...
def update_coverage_options(policy_id: str) -> str:
    """Предлагает варианты обновления информации о страховом покрытии."""
    # Логика для получения и предъявления вариантов обновления покрытия
    return f"Доступные варианты обновления покрытия для полиса {policy_id}: вариант1, вариант2."

@tool
def confirm_changes(policy_id: str) -> str:
    """Подтверждает изменения в страховом покрытии."""
    # Логика для подтверждения изменений через API или другой механизм
    return f"Изменения в полисе {policy_id} успешно подтверждены."
@tool
def notify_client(client_id: int, transaction_details: str) -> str:
    """Уведомление клиента о подозрительной транзакции."""
    # Логика для уведомления клиента через API или другой механизм
    return f"Клиенту {client_id} отправлено уведомление о подозрительной транзакции: {transaction_details}"

@tool
def request_additional_info(client_id: int, info_type: str) -> str:
    """Запрос дополнительной информации для верификации."""
    # Логика для запроса дополнительной информации через API или другой механизм
    return f"Запрошена дополнительная информация у клиента {client_id} для {info_type}"

@tool
def block_user_card(card_id: int, reason: str) -> str:
    """Блокировка карты пользователя."""
    # Логика для блокировки карты через API или другой механизм
    return f"Карта {card_id} заблокирована по причине: {reason}"
...
slovar = {
    'Название сценария': [
        ...
        '''Обработка подозрительной активности на счете''',
    ],
    'Условия входа': [
        '''Запрос пользователя связан с одной из перечисленных тем:
1) информация о страховом полисе
2) необходимость изменения условий страхования''',
        '''Запрос пользователя связан с одной из перечисленных тем:
1) подозрительные транзакции
2) уведомления о несанкционированных списаниях''',
    ],
    'Описание сценария': [
        '''Проверка данных клиента. Верификация статуса полиса. - Используй функцию verify_policy_status(). Обновление информации о страховом покрытии. - Предложи варианты с использованием функции update_coverage_options(). Подтверждение изменений. - Функция confirm_changes().''',
        '''Проверка активности на счете клиента. Уведомление клиента о подозрительной транзакции. - Используй функцию notify_client(). Запрос дополнительной информации для верификации. - Предложи варианты с использованием функции request_additional_info(). Блокировка карты при необходимости. - Используй функцию block_user_card(). Разоблачение и снятие блокировки при подтверждении легитимности транзакции.''',
    ],
    'Справочная информация': [
        '''
        Справочной информации нет для текущего сценария, используйте функции verify_policy_status(), update_coverage_options(), confirm_changes().
        ''',
        '''
        Справочной информации нет для текущего сценария, используйте функции notify_client(), request_additional_info(), block_user_card().
        ''',
    ],
}

df = pd.DataFrame(slovar)
assert df.shape == (2, 4), 'ОШИБКА ГЕНЕРАЦИИ ДАТАФРЕЙМА СЦЕНАРИЕВ'
